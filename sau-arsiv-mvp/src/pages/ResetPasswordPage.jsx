...
const ResetPasswordPage = () => {
  ...
  // URL'den token alınacak (örnek)
  const resetToken = 'example-reset-token';
  const userEmail = 'ogrenci@ogr.sakarya.edu.tr'; // API'den gelecek
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('Password reset successful:', { token: resetToken, newPassword: formData.password });
      setIsSuccess(true);
    } catch (error) {
      setErrors({ general: 'Şifre sıfırlanırken bir hata oluştu. Lütfen tekrar deneyin.' });
    } finally {
      setIsLoading(false);
    }
  };
